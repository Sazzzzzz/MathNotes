...
*   **定位文件**：
    *   所有笔记位于仓库的 `./note/` 目录下。
    *   根据主题定位目标子目录，识别 `main.tex`、章节文件及参考教科书（用户提供）。
...
*   **代码规范**
    *   确保生成的 LaTeX 代码语法正确，缩进清晰，无未闭合的环境或括号。
    *   使用类文件中已定义的环境（如 theorem、definition、proof），请勿使用未定义的环境。
    *   尽可能使用类文件或`main.tex`中已有的自定义命令与宏，保证风格一致。例如，使用 `\R` 表示实数集、`\norm{a}{b}` 表示范数等。
