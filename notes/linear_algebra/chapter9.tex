...
行列式本身就是迷一样的定义，不少教科书还信誓旦旦地试图讲解行列式的本质，只是弄巧成拙，令人忍俊不禁。

为了让这个概念更加神秘：如果我们认定行列式就是面积，不难发现行列式本身的定义是不依赖于内积的，与我们想象中的度量空间大相径庭。

% 交错线性形
% Gram阵，不知道为什么，张宇特别喜欢这个Gram矩阵，老拿他举例子
% 即使我们就此打住，也已经可以解决如三维空间中面积计算积分公式为什么那么难背诵了
% 外积与微分流形
\section{行列式的计算}
...
\begin{theorem}[矩阵行列式引理]
    ...
    \]
    \label{theorem:matrix-determinent-lemma}
\end{theorem}
...
在矩阵情况下还可以有：
\[
    \det(A+ UV^{\mathsf{T}}) = \det(I_{m} + V^{\mathsf{T}}A^{-1}U) \det A
\]

该定理可以用于快速计算与\(A\) 相差不大的矩阵的行列式。
\begin{example}[差一点单位阵行列式]
    ...
    \end{align*}
\end{example}
那最符合以上条件的行列式莫过于矩阵的特征多项式了！简单计算不难得到：
\begin{align*}
    \det(\lambda I - \bm{u}\bm{v}^{\mathsf{T}}) &= \lambda^{n-1} (\lambda - \bm{v}^{\mathsf{T}}\bm{u})\\
    \det(\lambda I - UV^{\mathsf{T}}) &= \lambda^{n-m} \det(\lambda I_{m}-V^{\mathsf{T}}U)
\end{align*}
不难看出形如\(A=\bm{u}\bm{v}^{\mathsf{T}}\) 的矩阵特征值为\(0\) 与\(\tr A = \bm{u}^{\mathsf{T}}\bm{v}\)。
低秩矩阵的特征值就是与其对应满秩方阵的特征值加若干重零。
