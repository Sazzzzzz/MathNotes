...
\begin{theorem}[矩阵行列式引理]
    ...
    \[
        \det(A +\bm {uv}^{\mathrm{T}})=(1+\bm {v}^{\mathrm{T}}A^{-1}\bm {u} ) \det(A)
    \]
\end{theorem}
...
\begin{proof}
    \[
        ...
        \begin{vmatrix}
            I & \bm{u}\\
            0 & 1-\bm{v}^{\mathrm{T}}\bm{u}
        \end{vmatrix}
    \]
    从而\[
        \det(I +\bm {uv}^{\mathrm{T}})=(1+\bm {v}^{\mathrm{T}}\bm {u})
    \]
...
\begin{example}[差一点单位阵行列式]
    \begin{align*}
        \begin{vmatrix}
            ...
            \vdots &\vdots & \ddots & \vdots \\
            a & a & \dots & b
        \end{vmatrix} &= \abs{(b-a)I+a \bm{1}\bm{1}^{\mathrm{T}}} \\
        &= (b-a)^{n} \left( 1+a \bm{1}^{\mathrm{T}}\bm{1} \cdot \frac{1}{b-a} \right) \\
        &= (b-a)^{n-1} ((n-1)a+b)
    \end{align*}
\end{example}
