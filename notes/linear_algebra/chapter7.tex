...
\subsubsection{如何计算正交补}
给定一组列向量\(\alpha_{1},\dots \alpha_{n}\)，如何计算其正交补呢？只需利用行空间与零空间的正交补关系。

令\(A = (\alpha_{1},\dots \alpha_{n})^{\mathsf{T}}\)，求解\(A \bm{x}=\bm{0}\) 得其零空间，
就是原向量组张成空间的正交补空间。
\subsection{自伴算子}

\subsubsection{定义}
\begin{definition}[自伴算子]
    如果一个算子 \(T \in \L(V)\) 满足 \(T = T^*\)，即对于所有 \(v, w \in V\) 都有
    \[
...
\begin{theorem}
    自伴算子的特征值都是实数，斜伴随算子的特征值是零或纯虚数。
\end{theorem}
\begin{proof}
    设 \(T\) 是自伴算子，\(Tv = \lambda v\)，则
    \[
        \lambda \norm{v}^{2} = \langle \lambda v,v \rangle = \langle Tv,v \rangle = \langle v, T^* v \rangle = \langle v, Tv \rangle = \langle v, \lambda v \rangle = \bar{\lambda} \norm{v}^{2}
    \]
    从而\(\lambda = \bar{\lambda}\)，即特征值是实数。
    斜伴随算子同理。
\end{proof}
其实矩阵里面特别喜欢玩这个\UseVerb{smile}
