...
\begin{definition}[域（抽象版）]
    ...
    \footcite[定义~2.2.20]{ChouXiangDaiShu2017}
\end{definition}
...
例如\((1,0,0,0,0,0,0,0,0)^{\mathsf{T}}\)表示。

我们记开关初始状态向量为\(B=(1,0,\dots,0)^{\mathsf{T}}\)，在第\(i\)
...
T_{9})\) 称作状态转移矩阵，\(x=(x_{1}, x_{2}, \dots, x_{9})^{\mathsf{T}}\)
于是求解灭灯问题就完全转化成了在有限域
\(\Z_{2}\) 上求解线性方程组\[
    Ax+B=0
\]
...
\(\overrightarrow{e_{n}}=[0,\dots,1,\dots,0]^{\mathsf{T}}\)，
...
\begin{quote}
    ...
    都能看成从\(\R\)到\(\R\)的线性映射。\footcite[p.~46]{LinearAlgebraDone2024}
\end{quote}
如果我们定义：
\[
    \begin{aligned}
...
\begin{theorem}[子空间的条件]
    ...
    \begin{enumerate}
        ...
        \item 对于加法封闭（closed under addition）：\(u, w \in U \implies u +
            w \in U\) 。
        \item 对于标量乘法封闭（closed under scalar
            multiplication）：\(a \in F\)  且 \(u \in U \implies au \in U\)。
    \end{enumerate}
