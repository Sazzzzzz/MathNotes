...
变换和数据固然是矩阵内禀的性质。尽管如此，我们也很难说矩阵的本质就是线性变换的一种表示。毕竟图论有邻接矩阵、置换里有置换矩阵，
矩阵和它的运算更像是一个哪里需要哪里搬的角色。

% todo: 矩阵“本质到底是什么”，未来可以参考以下回答
% https://www.zhihu.com/question/22237507/answer/1985502589501080176
% https://www.zhihu.com/question/19666954/answer/1985167626855522901 (type-(1,1) type-(0,2) difference)
...
\begin{itemize}
    ...
    \item （习题4）线性变换不严格单调降低向量组的秩。由此可以看出，可逆线性变换作为同构，不改变向量组的秩。
    \item （习题7）一维子空间有特征值
\end{itemize}
...
\subsection{变换大赏}
\subsubsection{剪切变换}
把矩阵的一行加到另一行，对应的变换就是剪切变换\UseVerb{smile}

对于向量空间 \(V\) 和子空间 \(W\)，固定 \(W\) 的剪切变换会将所有向量沿平行于 \(W\) 的方向平移。
广义的剪切变换包括所有的单位上三角阵和单位下三角阵。
\begin{align*}
    v &= w' + w \\
    L(v) &= Lw'+Lw = (w + Mw') + w'
\end{align*}

剪切变换可以说是保行列式不变的所有操作中乍一看最摸不着头脑的那个。
但实际上高斯消元法、证明平行四边形面积与同底等高的长方形相同、斜二测画法对\(y\)轴缩放后的变换、甚至文字的\emph{斜体}，都是剪切变换的应用。

\subsubsection{旋转变换}
特殊正交群 \(SO(n)\) 是所有旋转变换的集合。每个旋转变化都能分解成若干个二维平面上的旋转变换的组合。
\subsubsection{反射变换}
向量 \(\bm{x}\) 经子空间 \(W\) 反射后的向量为\(\bm{x} - 2\inner{\bm{x}}{\bm{n}}\bm{n}\)。
其中 \(\bm{n}\) 是 \(W\) 的单位法向量。该变换也称为 Householder 变换。
矩阵形式为 \(I - 2\bm{n} \bm{n}^{\mathsf{T}}\)

这玩意似乎在数值分析里有很多应用。不过我个人最喜欢的应用还是高中的时候快速计算点关于直线的反射\UseVerb{laugh}
...
\begin{proof}
    令\(u_1, \dots , u_{m}\) 是\(\nullspace T\) 的一个基，于是\(\dim
    \nullspace T = m\)，线性无关组 \(u_{1}, \dots ,u_{m}\) 可以被扩充为\(V\) 的一组基。
\end{proof}
...
此外要注意，尽管\(\dim V = \dim \nullspace T + \dim \range T\)，但构成\(V\)的一组基，
不是\(\range T\)的基加上\(\nullspace T\)的基（也即并非直和），
而是\(\nullspace T\)的基与\(\range T\)的基的原像。这个小问题将会在广义特征分解里痛击我们\UseVerb{sad}
...
\subsection{矩阵乘法}
矩阵乘法可以从元素、列/行、列乘行以及低秩分解四个角度理解：
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{resources/MatrixTimesMatrix.pdf}
    \caption{矩阵乘法的四种理解（图源：
        \href{https://github.com/kenjihiranabe/The-Art-of-Linear-Algebra}{Kenji Hiranabe, \emph{The Art of Linear Algebra}}，
    CC0 1.0）}
    \label{fig:matrix-times-matrix}
\end{figure}
\subsection{秩！}
% 可逆线性变换的逆也是线性变换，保持数乘和加法结构。这说明可逆线性变换本身就是一个同构。
\subsection{矩阵的计算}
\begin{quote}
    代数课程的一个任务就是教会读者使用许多运算系统。
...
\paragraph{对角阵}
\begin{align*}
    \diag(a_{1},\dots ,a_{n})^{k} &= \diag(a_{1}^{k},\dots,a_{n}^{k} )\quad k\in \N \\
    \diag(a_{1},\dots ,a_{n})\diag(b_{1},\dots ,b_{n}) &= \diag(a_{1}b_{1},\dots ,a_{n}b_{n})
\end{align*}

\section{向量空间的积与商}
\subsection{把子空间放大}
% TODO:  reference to chapter 2
...
线性泛函又称线性形式/余向量（covector）我们通过观察零空间理解线性泛函。线性泛函可以类比空间中的平面，其法向量就是对应的向量（Riesz 表示定理）
