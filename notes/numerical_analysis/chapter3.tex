...
Clenshaw 算法是Horner算法的推广：
\begin{theorem}[Clenshaw 算法]
    设函数序列 \(\phi_k(x)\) 满足线性递推关系：
    \[ \phi_{k+1}(x) = \alpha_k(x) \phi_k(x) + \beta_k(x) \phi_{k-1}(x) \]
    其中系数 \(\alpha_k(x)\) 和 \(\beta_k(x)\) 已知（通常情况下甚至与\(k, x\)无关）。
    为计算有限项级数 \(S(x) =
    \sum_{k=0}^n a_k \phi_k(x)\)，可利用如下递推公式：
    \begin{itemize}
        \item 令 \(b_{n+1}(x) = b_{n+2}(x) = 0\)。
        \item 逆向计算 \(b_k(x)\)：
            \[ b_k(x) = a_k + \alpha_k(x) b_{k+1}(x) + \beta_{k+1}(x)
            b_{k+2}(x), \quad k = n, n-1, \dots, 1 \]
        \item 最终级数和为：
            \[ S(x) = \phi_0(x) a_0 + \phi_1(x) b_1(x) + \beta_1(x)
            \phi_0(x) b_2(x) \]
    \end{itemize}
\end{theorem}

\begin{proof}
    将递推关系变形为 \(a_k = b_k - \alpha_k b_{k+1} - \beta_{k+1} b_{k+2}\)，
    代入级数 \(S(x) = \sum_{k=0}^n a_k \phi_k\)：
    \begin{align*}
        S(x) &= \sum_{k=0}^n (b_k - \alpha_k b_{k+1} - \beta_{k+1}
        b_{k+2}) \phi_k \\
        &= \sum_{k=0}^n b_k \phi_k - \sum_{k=0}^n \alpha_k b_{k+1}
        \phi_k - \sum_{k=0}^n \beta_{k+1} b_{k+2} \phi_k
    \end{align*}
    对后两项进行指标变换，并利用 \(b_{n+1} = b_{n+2} = 0\)：
    \begin{align*}
        S(x) &= \sum_{k=0}^n b_k \phi_k - \sum_{k=1}^n \alpha_{k-1}
        b_k \phi_{k-1} - \sum_{k=2}^n \beta_{k-1} b_k \phi_{k-2} \\
        &= b_0 \phi_0 + b_1 \phi_1 - b_1 \alpha_0 \phi_0 +
        \sum_{k=2}^n b_k (\phi_k - \alpha_{k-1} \phi_{k-1} -
        \beta_{k-1} \phi_{k-2})
    \end{align*}
    由于基函数满足 \(\phi_k - \alpha_{k-1} \phi_{k-1} - \beta_{k-1}
    \phi_{k-2} = 0\)，求和项消失。代入 \(b_0\) 的定义式 \(b_0 = a_0 + \alpha_0 b_1
    + \beta_1 b_2\)：
    \begin{align*}
        S(x) &= (a_0 + \alpha_0 b_1 + \beta_1 b_2) \phi_0 + b_1
        \phi_1 - \alpha_0 b_1 \phi_0 \\
        &= a_0 \phi_0 + b_1 \phi_1 + \beta_1 b_2 \phi_0
    \end{align*}
\end{proof}
% todo: 写出算法
\section{内积空间的最佳逼近}
...
因为高频的震荡才能避免非零值产生的巨大积分\footnote{参考\href{https://www.embeddedrelated.com/showarticle/152.php}{这篇文章}}
\begin{figure}[H]
    \centering
...
这里的多项式往往有自己的定义域。例如，要将Legendre 多项式拓展到任意\([a,b]\) 区间上，需要线性变换 \(x =
\frac{b-a}{2} t + \frac{a+b}{2}\) 把 \([a,b]\) 映射到 \([-1,1]\) 上。

\paragraph{Legendre 多项式}
