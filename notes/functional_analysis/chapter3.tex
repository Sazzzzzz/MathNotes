...
\(\R^n\) 中，向量不仅有“长度”（范数），还有“角度”（由内积定义）。
希尔伯特空间就是装备了内积的完备赋范线性空间（完备这里指的是从范数由内积诱导的Banach空间上完备）。

\section{Basic properties}

关于内积空间的基本定义、柯西-施瓦茨不等式（Cauchy-Schwartz inequality）、勾股定理、三角不等式等性质，
与线性代数中的内容基本一致，不再赘述。
...
平行四边形定则揭示了范数与内积之间的联系。如果一个范数满足平行四边形定则，
那么它一定是由某个内积诱导的。这是因为平行四边形定则可以推导出极化恒等式。
...
\begin{example}
    考虑\(L^{\infty }[0,1] \), 取\(f(x)= x\)，\(g(x)= x^2\)
    ...
    故\(L^{\infty }[0,1] \)上范数不满足平行四边形定则，不能由内积诱导。

    \(L^{\infty }[0,1] \)不是内积空间，这就解释了数值分析中最佳一致逼近与内积空间最佳逼近的区别。
\end{example}
...
\begin{proof}
    在闭空间上的证明很简单：用勾股定理即可。
    在闭凸集上，\(\forall x, y \in H\) 且\(x=P_{B}(x), y=P_{B}(y)\):
    \begin{align*}
        \Re \inner{x-\hat{x}}{b-\hat{x}} &\le 0 \\
        \Re \inner{y-\hat{y}}{b-\hat{y}} &\le 0
    \end{align*}
    取 \(b = \hat{y}\) 和 \(b = \hat{x}\)，相加得：
    \[
        \Re \inner{y-x-(\hat{y} - \hat{x})}{\hat{y} - \hat{x}} \geq  0
    \]
    从而：
    \[
        \|\hat{y} - \hat{x}\|^2 \leq \Re \inner{y - x}{\hat{y} - \hat{x}} \leq
        \|y - x\| \|\hat{y} - \hat{x}\|
    \]
    即\(\norm{x-y}\geq \norm{\hat{x}-\hat{y}}\)
\end{proof}
...
\begin{definition}[Orthogonality]
    ...
    \begin{enumerate}
        ...
        \item 若集合 \(S_1, S_2 \subset H\) 满足对任意 \(x \in S_1, y \in
            S_2\) 都有 \(x \perp y\)，则称 \(S_1\) 与 \(S_2\) 正交，记为
            \(S_1 \perp S_2\)。
        \item 定义 \(S\) 的\textbf{正交补}（orthogonal complement）为：
            \[
                S^\perp = \{y \in H : \inner{y}{x} = 0,
                \forall x \in S\}.
            \]
    \end{enumerate}
\end{definition}
...
\begin{proof}
    \begin{itemize}
        ...
        \item (5) 由(4) 有\(S \in S^{\perp\perp}
            \)即\(S^{\perp \perp \perp }
            \subset  S^\perp\)
            由(4) 有\(S^{\perp } \subseteq (S^{\perp })^{\perp
                \perp } = S^{\perp
            \perp \perp } \)
    \end{itemize}
\end{proof}
...
所以说，有限维空间有正交分解，无限维希尔伯特空间也有正交分解。那什么空间没有正交分解呢？不完备的内积空间。
\begin{example}
    考虑\(C[-1,1]\)上的内积\(\int_{-1}^{1} fg \d{x}\)。考虑：\[
        U = \left\{ f\in  C[-1,1] \mid f(0)=0 \right\}
    \]
    容易证明\(U^{\perp } = \{0\} \), \(C[0,1] \neq U \oplus U^{\perp }
    \)。这是因为\(C[0,1]\) 不是希尔伯特空间，其完备化为\(L^2[-1,1]\)。
\end{example}
...
下面的定理是希尔伯特空间理论中最重要的结果之一，它建立了抽象希尔伯特空间与序列空间 \(\ell^2\) 之间的联系。
...
\begin{theorem}[Expansion and Parseval's Identity]
    ...
    \begin{enumerate}
        \item \textbf{展开}：
            \[
                x = \sum_{\alpha \in A} \inner{x}{e_\alpha} e_\alpha.
            \]
        ...
        \item \textbf{帕塞瓦尔恒等式}：
            \[
                \|x\|^2 = \sum_{\alpha \in A} |\inner{x}{e_\alpha}|^2.
            \]
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    由于 \(\sum_{\alpha \in A} |\norm{x}{e_\alpha}|^2\) 收敛，
    对于任意 \(\epsilon > 0\)，存在 \(F_0 \in \mathcal{F}\) 使得对所有
    \(F \supset F_0\)，有
    \[
    ...
    \]
    这表明网 \((x_F)_{F \in \mathcal{F}}\) 是柯西网。由于 \(H\) 是完备的，
    该网收敛于某个元素 \(y \in H\)。
...
\begin{proof}
    ...
    如果基是无限的，设 \(B_1\) 和 \(B_2\) 是两个规范正交基。对于每个 \(e \in B_1\)，由广义Fouier展开知，
    集合 \(A_e = \{f \in B_2 : \inner{e}{f} \neq 0\}\) 是可数的。
    从而：\[
...
\begin{proof}
    ...
    \[
        ...
        \|Tx-Ty\|^2) = \frac{1}{4}(\|x+y\|^2 - \|x-y\|^2) =
        \inner{x}{y}.
    \]
\end{proof}
% 这好像是我第一次完整的写完一章连续的笔记:-o